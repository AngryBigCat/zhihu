...
<div role="tabpanel" class="tab-pane" id="zhuanlan">
    ...
    <div class="zhuan-item" >
        ...
        <div class="zhuan-right" >
            ...
            <div class="zhuan-bottom"> <span>{{ \App\User::find($val->id)->answers->count() }}</span> 回答 • 共 <span>{{ \App\User::find($val->id)->questions->count() }} </span>
            提问 • <span> {{ \App\User::find($val->id)->followers()->get()->count() }} </span>关注者
            </div>
        </div>
        @if (Auth::id() != $val->id) 
